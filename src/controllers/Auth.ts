...
class AuthController {
  ...
  async signIn(req: express.Request, res: express.Response) {
    const requestSchema = Joi.object({
      ...
      password: Joi.string().required().min(8).max(32).error(new Error("Valid password required."))
    });
    ...
    const userWithThisEmail = await User.findOne({ email }).populate("roles").lean().exec() as UserInterface | null;
    if (!userWithThisEmail) return res.status(400).send({ message: "Invalid user credentials." });

    const passwordIsValid = bcrypt.compareSync(password, userWithThisEmail.password);
    if (!passwordIsValid) return res.status(400).send({ message: "Invalid user credentials." });
