...
class UserController {
    ...
    async create(req: express.Request, res: express.Response) {
        const requestSchema = Joi.object({
            ...
            password: Joi.string().required().min(8).max(32).error(new Error("Your password must have a minimum length of 8 and a maximum of 32.")),
            roles: Joi.array().items(
                Joi.string().required().regex(mongoObjectId).error(new Error("Invalid role inside roles list."))
            ).required().error(new Error("At least 1 valid role required."))
        });
        ...
        for (const role of roles) {
            ...
            if (!roleExists) return res.status(400).send({ message: "One of the received roles doesn't exist." });
        }
